...
<script>
	function addrefinementvalue(name, value){
		$('#refinements').val($('#refinements').val() + '~' + name + '=' + value);
		$('#form').submit()
	}
</script>
...
<c:forEach items="${nav.refinementValues}" var="value" varStatus="iC">
  ...
  <c:if test="${iC.index < max}">
    <c:if test="${value.count > 0}">
      ...
      <li><a href="javascript:;"
        		onclick="addrefinementvalue('<c:out value="${nav.name}"/>', '<c:out value="${value.value}"/>')">
  ...
  <c:if test="${iC.index >= max}">
    <c:if test="${value.count > 0}">
      <c:set var="elements" value="${elements + 1}"/>
        <li><a style="display:none" class="hide${gc:md5(nav.name)}" href="javascript:;"
        		onclick="addrefinementvalue('<c:out value="${nav.name}"/>', '<c:out value="${value.value}"/>')">
        <str:truncateNicely upper="20"><str:capitalizeAllWords>${fn:toLowerCase(value.value)}</str:capitalizeAllWords></str:truncateNicely>
        <span class="count">(${value.count})</span> 
        </a></li>
  ...
  <c:if test="${fn:length(nav.refinementValues) >= max}">
    <li><a href="javascript:;" onclick="$('div', $(this)).toggle();$('.hide${gc:md5(nav.name)}').toggle()">
    		<span>more...</span>
