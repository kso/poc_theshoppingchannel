...
public class NavigationController extends AbstractController {
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest pRequest,
		...
		String action = ServletRequestUtils.getStringParameter(pRequest,"action");
		
		if ("signIn".equals(action)) {
			String email = ServletRequestUtils.getStringParameter(pRequest,"email", null);
			String password = ServletRequestUtils.getStringParameter(pRequest,"password", null);
		...
		Integer port = new Integer(getServletContext().getInitParameter(
			"bridgePort"));
		String key = getServletContext().getInitParameter("bridgeKey");
